...
public class CalendarUtil {
	...
	public static String getNextDay(String format){
        ...
        return DateUtils.getDateText(calendar.getTime(), format);
	}
	...
	public static Map<String,String> getMonthFirstEndDayTime(String year,String month){
		...
		String firstDayTime = DateUtils.COMMON.getDateText(calendar.getTime()) +" 00:00:00";
		
		calendar.set(Calendar.DATE, calendar.getActualMaximum(Calendar.DATE));//设置本月最大日期
		String lastDayTime = DateUtils.COMMON.getDateText(calendar.getTime()) +" 23:59:59";
	...
	public static List<String> getBetweenMonths(String startMonth ,String endMonth){
		...
		while(startCalendar.getTimeInMillis() <= endCalendar.getTimeInMillis()){
			months.add(DateUtils.getDateText(startCalendar.getTime(),"yyyy-MM"));
	...
	public static List<String> getBetweenDateStr(String startTime,String endTime){
		...
		try {
			Long startM = DateUtils.COMMON.getTextDate(startTime).getTime();
			Long endM = DateUtils.COMMON.getTextDate(endTime).getTime();
			...
			for(int i = 0; i <= result ; i++){
				startCalendar.add(Calendar.DATE,1);
				dateList.add(DateUtils.COMMON.getDateText(startCalendar.getTime()));
			}
	...
	public static List<String> getBetweenDateStr(String startTime,String endTime,String format){
		...
		try {
			...
			for(int i = 0; i <= result ; i++){
				startCalendar.add(Calendar.DATE,1);
				dateList.add(DateUtils.getDateText(startCalendar.getTime(),format));
			}
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static List<String> getPreNMonths(String curMonth ,Integer n){
		...
		while(tmpCalendar.getTimeInMillis() <= curCalendar.getTimeInMillis()){
			months.add(DateUtils.getDateText(tmpCalendar.getTime(),"yyyy-MM"));
	...
	public static boolean isDayEqual(Date date1,Date date2){
		...
		String date1Str = DateUtils.COMPAT.getDateText(date1);
		String date2Str = DateUtils.COMPAT.getDateText(date2);
	...
	public static void main(String[] args){
		...
		try {
			Date startTime = DateUtils.COMMON.getTextDate("2015-06-03 00:00:00");
			Date endTime = DateUtils.COMMON_FULL.getTextDate("2015-06-03 23:59:59");
			...
			String str = DateUtils.COMMON_FULL.getDateText(getPreNDayEnd(new Date(),1));
