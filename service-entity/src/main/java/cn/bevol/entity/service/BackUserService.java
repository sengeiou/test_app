...
import cn.bevol.mybatis.dao.BackUserMapper;
...
public class BackUserService extends BaseService {
	...
	public ReturnData addBlcakList(Long userId, Integer state, String description, Long startTime, Long endTime) {
		try {
			if (null != userId && userId > 0) {
				...
				UserBlackList.setCreateTime(new Date().getTime() / 1000);
				...
				if (state == 2) {
					...
					backUserMapper.insertUserBlackList(UserBlackList);
					// 永久
				} else if (state == 1) {
					backUserMapper.insertUserBlackList2(UserBlackList);
				}
				//发送xxj消息
	...
	public ReturnData removeBlcakList(Long userId) {
		try {
			if (null != userId && userId > 0) {
				backUserMapper.deleteBlackList(userId);
	...
	public ReturnData updateBlcakList(Long userId, Integer state, String description, Long startTime, Long endTime) {
		try {
			if (null != userId && userId > 0) { 
				...
				userBlackList.setUserId(userId);
				backUserMapper.updateBlackList(userBlackList);
	...
	public List<UserBlackList> getUserBlackList2() {
		try {
			List<UserBlackList> listUser = backUserMapper.getUserBlackList();
			return listUser;
		} catch (Exception e) {
			Map map=new HashMap();
	...
	public ReturnData getEssenceCommentAndXxsNum() {
		try {
			...
			//根据用户进行分组
			List<Map<String,Object>>  essenceCommentList= indexMapper.getNum(1);
			...
			//修行说的数量
			List<Map<String,Object>>  xxsList= indexMapper.getNum(2);
