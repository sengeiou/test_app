...
import cn.bevol.model.entity.UserPart;
import cn.bevol.mybatis.dao.ListsMapper;
import cn.bevol.mybatis.dao.TagStatiscalMapper;
...
public class TagStatiscalService {
    ...
    @Resource
    private TagStatiscalMapper tagStatiscalMapper;
    @Resource
    ...
    public Integer insertTags(){
        ...
        for (String s: findMap.keySet()) {
            ...
            if(!StringUtils.isEmpty(s)) {
                ...
                tagStatiscal.setUpdateDate(new Date().getTime()/1000);
                Integer tmp = tagStatiscalMapper.insertOrUpdateById(tagStatiscal);
                result+=tmp;
            }
    ...
    public Paged findTagStatiscal(Integer page,Integer pageSize){
        ...
        paged.setResult(tagStatiscalMapper.findByPage(paged));
        paged.setTotal(tagStatiscalMapper.selectTotal(paged));
        return paged;
    }
    ...
    public Map<String, Integer> listsTags() {
        ...
        int totalPage = listsMapper.selectTotal() / 200 + 1;
        for (int i = 1; i <= totalPage; i++) {
            paged.setCurPage(i);
            paged.setPageSize(200);
            List<Tags> tmpList = listsMapper.findTagByPage(paged);
            ls.addAll(tmpList);
        }
    ...
    public Paged getTagListsList(Integer tagId,Integer page,Integer pageSize) {
        ...
        paged.setPageSize(pageSize);
       paged.setCurPage(page);
        paged.setTotal( listsMapper.selectTotalByTag(tagId));
        paged.setResult(listsMapper.getListsByTagId(tagId,page,pageSize));
        return paged;
    }
