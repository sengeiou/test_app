...
import cn.bevol.model.entity.UserPartDetail;
import cn.bevol.model.entity.UserPartDetailText;
    ...
    public ReturnData syncState() {
        try {
            ...
            List<EntityApplyGoods2> entityApplyGoodsList = mongoTemplate.find(
                    new Query(Criteria.where("isSync").ne(1).and("hidden").is(0)),
            ...
            if(entityApplyGoodsList.size() >0 ) {
                ...
                for(EntityApplyGoods2 entityApplyGoods:entityApplyGoodsList) {
                    if(entityApplyGoods.getActiveState() !=null && ArrayUtils.contains(syncActionState, entityApplyGoods.getActiveState())) {
                        //更新未中奖的用户
