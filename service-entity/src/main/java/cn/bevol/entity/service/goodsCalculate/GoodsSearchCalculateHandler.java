...
public class GoodsSearchCalculateHandler implements GoodsCalculateI {
	...
	public void createSql(){
		...
		if(!StringUtils.isBlank(ruleCpsIds)){
			...
			this.updateSql="update hq_goods_search set cps='"+this.goods.getCps()+"',cps_search='"+newCps+"',category="+goods.getCategory()+",safety_1_num="+safety_1_num+",tag_ids='"+tag_ids+"',update_time="+new Date().getTime()/1000+",rule_cps_ids='"+ruleCpsIds+"',p_category="+pCategory+" where goods_id="+goods.getId();
			insertValues=" values("+goods.getId()+",'"+this.goods.getCps()+"','"+newCps+"',"+goods.getCategory()+","+safety_1_num+",'"+tag_ids+"',"+new Date().getTime()/1000+",'"+ruleCpsIds+"',"+pCategory+")";
		}else{
			ruleCpsIds=null;
			this.updateSql="update hq_goods_search set cps='"+this.goods.getCps()+"',cps_search='"+newCps+"',category="+goods.getCategory()+",safety_1_num="+safety_1_num+",tag_ids='"+tag_ids+"',update_time="+new Date().getTime()/1000+",rule_cps_ids="+ruleCpsIds+",p_category="+pCategory+" where goods_id="+goods.getId();
			insertValues=" values("+goods.getId()+",'"+this.goods.getCps()+"','"+newCps+"',"+goods.getCategory()+","+safety_1_num+",'"+tag_ids+"',"+new Date().getTime()/1000+","+ruleCpsIds+","+pCategory+")";
