...
public class CacheService {
	 ...
	 * @return
	 */
	public ReturnData redisCacheList(int pager, int pageSize) {
		try {
			int skip = 0;
			if (pager > 1) {
				skip = (pager - 1) * pageSize;
			}
			// List<RedisCache>
			// rcList=backGoodsMapper.getRedisList(skip,pageSize);
			List<Map> listMap = CACHE_NAME.listMap;
			return new ReturnData(listMap);
		} catch (Exception e) {
			logger.error(
					"method:CacheService.redisCacheList arg:{" + "	}" + "   desc:" + ExceptionUtils.getStackTrace(e));
		}
		return ReturnData.ERROR;
	}
