...
public class StaticFindService implements StaticInfoService {
	...
	public Boolean  findStatic(String id) throws IOException {
		...
		try {
			...
			for (Map<String, String> info : json) {
				...
				Find find=	findMapper.getById(Long.parseLong(id));
				if(find!=null){
					...
					if(org.apache.commons.lang.StringUtils.isEmpty(tag.trim())){
					...
					if(result){
						String filePath = PropertyUtils.getStringValue("static.html.root")+"/"+platform+"/"+id+".html";
	...
	private Map<String, Object> prepareIndustryStaticInfo(String id){
		Industry industry = industryMapper.findById(Long.parseLong(id));
	...
	public Boolean  industryStatic(Integer id) throws ClientProtocolException {
		...
		try {
			...
			for (Map<String, String> info : json) {
				...
				String[] imgs =  ConfUtils.imgs;
				String img = imgs[(new Random().nextInt(2))];
				Industry industry=	industryMapper.findById(id);
				if(industry!=null){
	...
	private List<Find> findByPage(Integer page) {
		...
		return findMapper.findByPage(paged);
	}
	...
	private List<Industry> industryList(Integer page) {
		...
		paged.setCurPage(page);
		paged.addOrderBy("id", "desc");
		return industryMapper.industryByPage(paged);
	}
	/***
	 * 初始化发现线程
	 */
	public Boolean initFindStatic(int arg){
		if(arg ==1 ||arg ==8) {
			...
			int tatolPage = findMapper.selectTotal() / 20 + 1;
				...
				public void run() {
					while (true) {
						try {
							...
							int tatolPage =  industryMapper.selectTotal()/20 +1;
							for (int i = 1; i <= tatolPage; i++) {
								queue.put(i);
							}
							int page = 1;
	...
	public Map<String, Integer> findTags() {
		...
		int totalPage = findMapper.selectTotal() / 200 + 1;
		for (int i = 1; i <= totalPage; i++) {
			paged.setCurPage(i);
			paged.setPageSize(200);
			List<Tags> tmpList = findMapper.findTagByPage(paged);
			ls.addAll(tmpList);
		}
	...
	public Paged getTagFindList(Integer tagId,Integer page,Integer pageSize) {
		...
		paged.setCurPage(page);
		paged.setTotal(findMapper.selectTotalByTag(tagId));
		paged.setResult(findMapper.getFindByTagId(tagId,page,pageSize));
		return paged;
	}
