...
import cn.bevol.entity.service.statistics.CommenStatisticsService;
...
public class SkinService extends BaseService {
    ...
    @Autowired
    private SkinInterpretationMapper skinInterpretationMapper;
	...
	public Map findSkinAndGoods(final String skin) {
		return new CacheableTemplate<Map>(cacheProvider) {
			...
			protected Map getFromRepository() {
				try {
					SkinInterpretation skinInterpretation = null;
					List<LikeGoods> likeGoods = null;
					String like_good = "";
    ...
    private ReturnData saveSurvey(long userId, SkinTestResult suveryid) {
        ...
        if (rsl.length() == 4) {
            ...
            if (StringUtils.isBlank(user.getResult())) {
            	...
            	userService.addScore(userId, ScoreOpt.FINISHEDSKIN);
       ...
       // curSuvery
        //保存肤质
        return new ReturnData(return_date);
    }
