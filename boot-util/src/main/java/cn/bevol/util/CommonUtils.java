...
import cn.bevol.util.email.CheckSumBuilder;
import cn.bevol.util.http.HttpClientFactory;
...
public class CommonUtils {
    ...
    public static String getMd5(String plainText) {
     ...
     public static String setLoginCookie(HttpServletResponse response,long id) {
        ...
        String password = 12+"219dv"+DateUtils.nowInMillis();//"1234567";//密码
    	...
    	String cookes=id+"_"+getMd5(DateUtils.nowInMillis()+"");
        String loginCode="logincode";
	...
	public static long getLoginCookieById2(HttpServletRequest request) {
		...
		if(str!=null) {
			...
			if((Long.parseLong(time)+loginTime)>DateUtils.nowInSeconds())
				return Long.parseLong(id);
		}
		}catch (Exception ex){
		}
	...
	public static void setLoginCookieId(HttpServletResponse response,long id,Integer maxtime) {
		String msg = id+""+idcode+""+DateUtils.nowInSeconds(); //动态cookie
	...
	public static void sendEmail(String toEmail,String subject,String content) throws EmailException {
		...
		String smtpPort=eh.get("smtp_port");
	...
	private static final String
			APP_KEY="fc6222233a07c1c2ca338e97d56782e2";
	//网易云信分配的密钥，请替换你在管理后台应用下申请的appSecret
	private static final String APP_SECRET="5ef1167e15b7";
	//随机数
	private static final String NONCE="123456";
	//短信模板ID
	private static final String TEMPLATEID="3882439";
	/*//手机号
	private static final String MOBILE="13469958973";*/
	//验证码长度，范围4～10，默认为4
	private static final String CODELEN="4";

	@LogMethod
	public static Map send163Email(String phone, String vcode,long id){
		Map resultMap=new HashMap();
		resultMap.put("send",-1);
		resultMap.put("id",id);
		try{
			...
			nvps.add(new BasicNameValuePair("authCode",vcode));
			httpPost.setEntity(new UrlEncodedFormEntity(nvps, "utf-8"));

			// 执行请求
			HttpResponse response = httpClient.execute(httpPost);
			String resultStr=EntityUtils.toString(response.getEntity(), "utf-8");
			resultMap=JsonUtils.toMap(resultStr);
			if(Integer.parseInt(resultMap.get("code")+"")==200){
                resultMap.put("send",0);
            }
			return resultMap;
		}catch (Exception e){
			e.printStackTrace();
    ...
    public static String timeStampCastToDate(Long timeStamp){
    	if(null!=timeStamp && timeStamp>0){
            ...
            String date = new SimpleDateFormat("yyyy年MM月dd号").format(new Date(timestamp));
            return date;
    	}
    	return "";
    }
