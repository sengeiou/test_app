<url>
    ...
    <loc>https://m.bevol.cn/topic/user_part/${list.id?c}.html</loc>
    ...
    <data>
        ...
        <display>
            ...
            <wap_url>https://m.bevol.cn/topic/user_part/${list.id?c}.html</wap_url>
            ...
            <author>
                ...
                <name>${list.userBaseInfo.nickname!}</name>
            ...
            <thumbnail>https://img1.bevol.cn/user_part/lists/${list.image}</thumbnail>
            <content>
                ...
                <items>
                    ...
                    <data><#list list.contents as item>${item.content!} </#list></data>
                    <!-- data: 内容，最少出现0次 最多出现1次，类型为字符串 -->
                    <image>
                        <!-- image: 正文图片，最少出现0次 最多出现1次 -->
                        <src>https://img1.bevol.cn/user_part/lists/${list.image}</src>
            ...
            <content_type>评测</content_type>
            <ugc_info>
                <!-- ugc_info：【必选】文章UGC热度信息，最少出现1次，最多出现1次，类型为字符串 -->
                <scan_count>${list.hitNum}</scan_count>
                <!-- scan_count：【必选】阅读量，没有填0，最少出现1次，最多出现1次，类型为字符串 -->
                <like_count>${list.likeNum}</like_count>
                <!-- like_count：【必选】点赞量，没有填0，最少出现1次，最多出现1次，类型为字符串 -->
                <dislike_count>0</dislike_count>
                <!-- dislike_count：【必选】点踩量，没有填0，最少出现1次，最多出现1次，类型为字符串 -->
                <comment_count>${list.commentNum}</comment_count>
                <!-- comment_count：【必选】评论量，没有填0，最少出现1次，最多出现1次，类型为字符串 -->
                <share_count>0</share_count>
                <!-- share_count：【必选】分享量，没有填0，最少出现1次，最多出现1次，类型为字符串 -->
                <collection_count>${list.collectionNum}</collection_count>
                <!-- collection_count：【必选】收藏量，没有填0，最少出现1次，最多出现1次，类型为字符串 -->
            </ugc_info>
            <goodsnum>${list.goods?size}</goodsnum>
            <!-- goodsnum：文章提及的商品数量，最少出现1次，最多出现1次，类型为字符串 -->
        <#list list.goods as product>
            <goods_list>
                <!-- goods_list: 【可选】（尽量填全，可优化检索效果）goods_list标记一个商品信息的开始和结束，一篇文章中提及多个商品，则填写多个goods_list（最少出现1次，不限制最多出现次数 -->
                <category_firstclass>美妆</category_firstclass>
                <!-- category_fisrtclass: 【可选】(填写会优化检索效果)商品一级分类， 例：数码，最少出现0次 最多出现1次，类型为字符串 -->
                <brand>${product.title}</brand>
                <!-- brand: 【可选】(填写会优化检索效果)商品品牌， 例：小米，最少出现0次 最多出现1次，类型为字符串 -->
                <#if product.image?? && product.image !="">
                    <goods_image>https://img0.bevol.cn/Goods/source/${product.image!}</goods_image>
                <#else/>
                    <goods_image>https://img0.bevol.cn/Goods/default.png</goods_image>
                </#if>
                <!-- goods_image：【可选】商品配图，最少出现1次，不限制最多出现次数 -->
            </goods_list>
        </#list>
        </display>
    </data>
