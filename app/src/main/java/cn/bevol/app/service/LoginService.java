...
import cn.bevol.model.ConsumerTest;
...
public class LoginService extends BaseService {
    ...
    public int register(UserInfo userInfo) {
        userInfo.setRole(3);
        int i = userInfoOldMapper.register(userInfo);
        return i;
    }
    ...
    public ReturnData wxLogin2_6(String unionid, String wxmbopenid,
        ...
        try {
            ...
            if (wxrd.getRet() == 1 && rdp.getRet() == 1) {
                //舍弃手机号码或者邮箱
                int j = userInfoOldMapper.updateField("hidden", 3, "id", puserInfo.getId());
                userInfo = (UserInfo) wxrd.getResult();
            ...
            if (userInfo != null && userInfo.getId() != null && userInfo.getId() > 0) {
                i = userInfoOldMapper.updateOne(userInfo);//登录
            } else {
    ...
    public ReturnData accountLogin2_6(String account, String password) {
        try {
            if (!StringUtils.isBlank(account) && !StringUtils.isBlank(password)) {
                ...
                if (account.indexOf("@") == -1) {
                    //手机登录
                    userInfo = userInfoOldMapper.phoneLogin(account, MD5Utils.encode(password));
                } else {
                    //邮箱登录
                    userInfo = userInfoOldMapper.emailLogin(account, MD5Utils.encode(password));
                }

                if (userInfo != null) {
                    userService.userInit(userInfo);
                    //更新登录时间
                    userInfoOldMapper.updateField("logintime", new Date().getTime() / 1000, "id", userInfo.getId());
        ...
        try {
            ...
            if (vc == null || curTime > (vc.getCreateStamp() + 60)) {
                ...
                if (sendsms == 1) {
                    ...
                    if (account.indexOf("@") == -1) {
                        //发送短信
                        aliyunService.sendVcode(account, vc.getVcode(), type,vc.getId());
                    } else {
                        //发送邮件
    ...
    private ReturnData getVcode(String account, int type, UserInfo userInfos,Integer vcodeType) {
        try {
            //VerificationCodeEntity vc = null;
            VerificationCode vc = null;

            //账号格式,是否注册过等验证
            ReturnData rdp=this.accountValid(account,userInfos,type);
            if(rdp.getRet()!=0){
                return rdp;
            }
            ...
            return this.validAccountSend(vc,account,type,vcodeType);
        } catch (Exception e) {
            Map map = new HashMap();
    ...
    public ReturnData accountValid(String account,UserInfo userInfos,int type){
        ReturnData rdp=null;
        UserInfo userInfo=null;
        if (account.indexOf("@") == -1) {
            //手机
     ...
     * @return
     */
    public ReturnData getVcode2_9_2(String account, int type, UserInfo userInfo,Integer vcodeType) {
        try {
            //VerificationCodeEntity vc = null;
            VerificationCode vc = null;
            //账号格式,是否注册过等验证
            ReturnData rdp=this.accountValid(account,userInfo,type);
            if(rdp.getRet()!=0){
                return rdp;
            }
    ...
    public ReturnData validAccountSend(VerificationCode vc,String account,int type,Integer vcodeType) {
        long curTime = new Date().getTime() / 1000;
        if (vc == null || curTime > (vc.getCreateStamp() + 60)) {
            //重新获取验证码
            //vc = new VerificationCodeEntity(account, type);
            ...
            if (sendsms == 1) {
                ...
                if (account.indexOf("@") == -1) {
                    //判断号码是否存在
                    VerificationCode canVc=verificationCodeMapper.findByPhone(account);
                    if(null!=canVc && canVc.getId()>0){
                        return UserInfo.ERRER_ACCOUNT_NOT_FOUND;
                    }
                    Map<String,Object> sendInfo=new HashMap<String,Object>();
                    if(null==vcodeType){
                        //发送阿里云/网易云短信
                        sendInfo = this.sendSMS(vc, account, type);
                    }else{
                        if(vcodeType==1){
                            //发送语音
                            aliyunService.sendVoiceVcode(account,vc.getVcode());
                        }else if(vcodeType==2){
                            //发送阿里云/网易云短信
                            sendInfo = this.sendSMS(vc, account, type);
                        }
                    ...
                    //记录短信是否发送成功
                    if(null!=sendInfo && null!=sendInfo.get("send") && Integer.parseInt(sendInfo.get("send")+"")!=0){
                        verificationCodeMapper.updateSendState(Long.parseLong(sendInfo.get("id")+""),Integer.parseInt(sendInfo.get("send")+""));
                    }
                } else {
                    //发送邮件
                    aliyunService.sendEmailVcode(account, vc.getVcode(), type);
    ...
    private Map<String, Object> sendSMS(VerificationCode vc, String account, int type) {
        //发送阿里云短信
        /*ConsumerTest consumerTest=new ConsumerTest();
        consumerTest.sendMsg(account, vc.getVcode(), type,vc.getId());*/
        Map<String, Object> sendInfo=new HashMap<>();
        sendInfo=aliyunService.sendVcode(account, vc.getVcode(), type,vc.getId());
        if(null!=sendInfo.get("send") && Integer.parseInt(sendInfo.get("send")+"")!=0){
            //如果阿里云短信失败,调用网易云短信
            sendInfo= CommonUtils.send163Email(account,vc.getVcode(),vc.getId());
        }
        return sendInfo;
    }

    //图片验证码对比验证
    public ReturnData compareImgVcode(HttpServletResponse response, HttpServletRequest request, String imgVcode, String account, Integer type, UserInfo userInfo,Integer vcodeType) {
        try {
            if (StringUtils.isNotBlank(account) && StringUtils.isNotBlank(imgVcode)) {
                ...
                if (StringUtils.isBlank(sessionImgVcode)) {
                    return new ReturnData(-1,"session没有返回验证码");
                }
                ...
                return getVcode(account, type, userInfo,vcodeType);
            }
        } catch (Exception e) {
            Map map = new HashMap();
    ...
    public ReturnData findNickname(String val) {
        try {
            ...
            //验证用户名
            UserInfo userInfo = userInfoOldMapper.findFeild("nickname", val);
        ...
        try {
            ...
            if (i > 0) {
                ...
                if(StringUtils.isNotBlank(phone)){
                    //update验证码表最近的一次获取注册验证码的状态
                    verificationCodeMapper.updateSendStateByPhone(phone,5,0);
                }
    ...
    public ReturnData restPassword(String account, String password) {
        try {
            ...
            if (!StringUtils.isBlank(phone)) {
                //重置密码
                i = userInfoOldMapper.restPassword2(phone, MD5Utils.encode(password));
            } else {
                i = userInfoOldMapper.restPassword3(email, MD5Utils.encode(password));
            }
            ...
            if (i > 0) {
                ...
                if (!StringUtils.isBlank(phone)) {
                    //查询用户
                    userInfo = userInfoOldMapper.findFeild("phone", account);
                } else {
                    userInfo = userInfoOldMapper.findFeild("email", account);
                }
                //清空密码
