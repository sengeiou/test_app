...
public class GoodsCategoryCalculateHandler implements GoodsCalculateI {
	...
	public void polyCategoryCal(){
		try{
			...
			//更新  
			updateSql2="update hq_goods_poly_category set category_id="+goods.getCategory()+",update_time="+new Date().getTime()/1000+",exist_category_ids="+0+" where goods_id="+goods.getId();
			//多分类
			if(this.num>1 && !StringUtils.isBlank(categoryIds)){
				categoryIds=categoryIds.substring(0,categoryIds.length()-1);
				updateSql2="update hq_goods_poly_category set category_id="+goods.getCategory()+",category_ids='"+categoryIds+"',update_time="+new Date().getTime()/1000+",exist_category_ids="+1+" where goods_id="+goods.getId();
			}
			...
			if(this.num>1 && !StringUtils.isBlank(categoryIds)){
				...
				insertValues2=" values("+this.goods.getId()+","+goods.getCategory()+",'"+categoryIds+"',"+1+","+new Date().getTime()/1000+")";
				...
				insertValues2=" values("+this.goods.getId()+","+goods.getCategory()+","+0+","+new Date().getTime()/1000+")";
				insertSql2=insertKeys2+insertValues2;
				isSql=true;
			}else{//不满足条件 不进行分类
				isSql=false;
