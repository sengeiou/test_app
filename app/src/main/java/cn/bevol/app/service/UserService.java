...
import cn.bevol.app.dao.mapper.BackUserOldMapper;
import cn.bevol.app.dao.mapper.DoyenOldMapper;
import cn.bevol.app.dao.mapper.EntityFindOldMapper;
...
public class UserService extends BaseService {
    ...
    @Autowired
    private DoyenOldMapper doyenMapper;
    ...
    @Autowired
    BackUserOldMapper backUserMapper;
    ...
    public ReturnData getVcode(String phone, int type) {
        try {
            ...
            if (vc == null || curTime > (vc.getCreateStamp() + 60)) {
                ...
                if(sendsms==1) {
                    aliyunService.sendVcode(phone, vc.getVcode(), type,vc.getId());
                    vc.setVcode(null);
                }
    ...
    public void userInit(UserInfo userInfo) {
        ...
        //判断是否被禁言
        UserBlackList userBlack = this.getUserBlackById(userInfo.getId());
    ...
    public UserBlackList getUserBlackById(final Long userId) {
        return new CacheableTemplate<UserBlackList>(cacheProvider) {
            ...
            protected UserBlackList getFromRepository() {
                try {
                    UserBlackList User = backUserMapper.getUserBlackById(userId);
                    return User;
                } catch (Exception e) {
                    Map map=new HashMap();
    ...
    /**
     * 保存用户ip地址(记录用户最近10次的情况)
     * @param userId
     * @param ip
     * @author ksy
     * @return
     */
    public ReturnData addUserLocation(UserInfo userInfo,String guestIP,Double lng,Double lat) {
        boolean flag=false;
        ...
        if(flag){
    		...
    		userLocation.setIp(guestIP);
    		userLocation.setLocation(new Double[] {lng,lat});
        	 //在后台设置访问时间
        	 userLocation.setVisitTime(System.currentTimeMillis()/1000);
        	 Criteria cri= Criteria.where("id").is(userInfo.getId());
             Query query=new Query(cri);
             UserInfo tempUserInfo=mongoTemplate.findOne(query, UserInfo.class);
             if(tempUserInfo!=null) {
            	 List<UserLocation> userLocations = tempUserInfo.getUserLocations();
            	 if(userLocations!=null&&userLocations.size()>0) {
            		 //每天只保存一次用户地理位置
            		 UserLocation temp = userLocations.get(userLocations.size()-1);
            		 Calendar lastVisitDate = Calendar.getInstance();
            		 lastVisitDate.setTimeInMillis(temp.getVisitTime()*1000);
            		 Calendar currentVisitDate = Calendar.getInstance();
            		 currentVisitDate.setTimeInMillis(userLocation.getVisitTime()*1000);
            		 //取出上次保存用户地理位置的时间和当前访问时间，如果不是同一天则保存当前访问
            		 Boolean isSameDay  = DateUtils.isSameDay(lastVisitDate, currentVisitDate);
            		 if(!isSameDay) {
            			 mongoTemplate.updateFirst(query, new Update().push("userLocations").slice(-10).each(userLocation), UserInfo.class,"user_info");
                         return ReturnData.SUCCESS;
            		 }
            	 }else {
            		 mongoTemplate.updateFirst(query, new Update().push("userLocations").slice(-10).each(userLocation), UserInfo.class,"user_info");
