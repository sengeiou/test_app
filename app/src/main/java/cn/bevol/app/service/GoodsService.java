...
public class GoodsService extends BaseService {
	...
	@Autowired
	GoodsExtOldMapper goodsExtMapper;
	...
	public List<GoodsEffectUsed> getEffectDeatilByGoodsId(final Goods goods, final List<Composition> cps) {
		try {
			...
			List<GoodsEffectUsed> geus=new ArrayList<GoodsEffectUsed>();
			geus=getGoodsEffectUsedByCategoryId(goods.getCategory());
			if(geus!=null) {
				//定位成分表字段
		...
		return new ArrayList<GoodsEffectUsed>();
	}
	...
	public void soapCps(List<GoodsEffectUsed> geus, Goods goods, List<Composition> cps){
		...
		if(goods.getCategory()==6 || goods.getCategory()==29 || goods.getCategory()==38){
			//2.cps含皂基的成分
			List<Integer> soapCpsList=compositonService.soapCps();
			//是否含有A类成分
			List<Integer> Alist=compositionOldMapper.selectAcomposition();
			...
			List<Integer> cpdsA= new ArrayList<Integer>();
			List<Composition> comps= new ArrayList<Composition>();
			Map<Long,Long> uqcps=new HashMap<Long,Long>();
			//产品成分中仅含有一种皂基成分即可标识此产品含皂基
			for(int i=0;i<cps.size();i++){
				for(int j=0;j<soapCpsList.size();j++){
					System.out.println(cps.get(i).getId());
					System.out.println(cps.get(i).getId().getClass());
					System.out.println(soapCpsList.get(j));
					System.out.println(soapCpsList.get(j).getClass());
					if(cps.get(i).getId().equals(soapCpsList.get(j).longValue())){
						cpdsIds.add(Long.valueOf(soapCpsList.get(j)+""));
						comps.add(cps.get(i));
					}
				}
				//皂基判断的第二种情况
				...
				for(int k=0;k<Alist.size();k++){
					if(cps.get(i).getId().equals(Alist.get(k).longValue())){
						cpdsA.add(Alist.get(k));
					}
				}
			}
			if(cpdsA.size()>0){
				//查询B类成分
				List<Integer> Blist=compositionOldMapper.selectBcomposition();
				for(int m=0;m<Blist.size();m++){
					for(int n=0;n<cps.size();n++){
						//B类产品排名必须在成分前5
						if(cps.get(m).getId().equals(Blist.get(n).longValue())&&n>=5){
							comps.add(cps.get(n));
							Long cpsId=Blist.get(m).longValue();
							cpdsIds.add(cpsId);
						}
					}
				}
	...
	public ReturnData<GoodsExplain> getGoodsExplain(final String mid, final UserInfo userInfo) {
		...
		ReturnData rd = new cn.bevol.app.cache.CacheableTemplate<ReturnData>(cacheProvider) {
			@Override
	 ...
	 * @return
	 */
	public List findGoodsMidByPage(cn.bevol.app.dao.db.Paged goodsCondition) {
