			...
			if(commentType[j]==1){
				for(var k=0;k<efficientType.length;k++){
					...
					if(efficientType[k]==1){
						//有效一级
						//评论
						efficientCount=db[entityName].count({"content":{$exists:true},"hidden":0,"deleted":0,"createStamp":{$gt:gt,$lt:lt},$or:[{$and:[{"pid":0}]},{$and:[{"pid":{$exists:false}}]}]});
