 ...
 */
public class ItemsSearchRequest extends BaseTaobaoRequest<ItemsSearchResponse> {
	
	

	/** 
	* 商品是否为虚拟商品true：是虚拟商品false：不是虚拟商品
	 */
	private Boolean auctionFlag;

	/** 
	* 商品是否为自动发货true：自动发货false：非自动发货
	 */
	...
	* 商品30天内最大销售数，设置此条件表示搜索结果里的商品，最近成交量必须小于等于设置的end_volume
	...
	/** 
	* 需返回的商品结构字段列表。可选值为Item中的以下字段：num_iid,title,nick,pic_url,cid,price,type,delist_time,post_fee；多个字段之间用“,” 分隔。如：num_iid,title。新增字段location（卖家地址，可以分别获取location.city,location.state）；score(卖家信用等级数),volume(最近成交量)新增字段：has_discount, num, is_prepay, promoted_service,list_time
	 */
