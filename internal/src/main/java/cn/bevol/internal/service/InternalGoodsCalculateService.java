...
public class InternalGoodsCalculateService {
    ...
    public void goodsCalculateLocalCache(){
        try {
            goodsService.setDomesticInternationalDataType();
            allGoodsSkin=goodsOldMapper.getAllGoodsSkin();
            allCommonGoodsCategory=goodsOldMapper.getAllCommonGoodsCategory();
            allSpecialCategory=goodsOldMapper.getAllSpecialCategory();
            allEnglishCategory=goodsOldMapper.getAllEnglishCategory();
            allGoodsTag=goodsOldMapper.getAllTag();
            goodsRules=goodsOldMapper.getAllRule();
            allGoodsCategory=goodsOldMapper.getAllGoodsCategory();
            ...
            List<Composition> allCpsList = compositionOldMapper.getAll();
            for(int i=0;i<allCpsList.size();i++) {
                ...
                if(!StringUtils.isBlank(usedsString)) {
                    List<Used> useds=compositionOldMapper.getUsedsByUid(usedsString);
                    cp.setUseds(useds);
                }
            ...
            //所有成分
            tagsByIsMain=goodsOldMapper.getAllTagComposition();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void excSql(StringBuffer skinBuffer,StringBuffer tagBuffer,StringBuffer safetyBuffer,StringBuffer polyBuffer,StringBuffer polysBuffer,StringBuffer searchBuffer,String skinSql,String safetySql,String polysKey,String polyKey,String tagSql,String searchSql){
        ...
        if(!StringUtils.isBlank(skinVal)){
            ...
            goodsOldMapper.insert(insertSkin);
        }
        ...
        if(!StringUtils.isBlank(tagVal)){
            ...
            goodsOldMapper.insert(insertTag);
        }
        ...
        if(!StringUtils.isBlank(safetyVal)){
            ...
            goodsOldMapper.insert(insertSafety);
        }
        ...
        if(!StringUtils.isBlank(polyVal)){
            ...
            goodsOldMapper.insert(insertPoly);
        }
        ...
        if(!StringUtils.isBlank(polysVal)){
            ...
            goodsOldMapper.insert(insertPolys);
        }
        ...
        if(!StringUtils.isBlank(searchVal)){
            ...
            goodsOldMapper.insert(insertSearch);
        }
    }
}
