...
public class BackIndexRecommendService extends BaseService {
	private static Logger logger = LoggerFactory.getLogger(BackIndexRecommendService.class);
		...
		try {
			...
			if (null != listComment && listComment.size() > 0) {
				for (Comment comment : listComment) {
					...
					List<UserInfo> user=userInfoOldMapper.findUserinfoByIds(comment.getUserId()+"");
					//用户头像
					...
					if (null != entityGoods) {
						...
						EssenceComment ec = indexOldMapper.findByGoodsId(entityGoods.getId());
						//读缓存获取精选点评
			...
			allMap.put("createStamp", DateUtils.nowInMillis() / 1000);
		...
		try {
			...
			if (tname.equals("userPart")) {
				...
				Comment comment = mongoTemplate.findOne(new Query(Criteria.where("id").is(id)), Comment.class,
						"entity_comment_goods");
				UserInfo user=userInfoOldMapper.findFeild("id", comment.getUserId()+"");
				//用户头像
	...
	public ReturnData saveEntity(Long createStamp, String content, String pageName) {
		...
		try {
			...
			if (entity != null && msps != null && pageName != null) {
				...
				Long updateStamp = DateUtils.nowInMillis() / 1000;
