...
import cn.bevol.statics.dao.mapper.*;
...
public class SeoService {
    ...
    @Resource
    private SeoRecordOldMapper seoRecordOldMapper;
    @Resource
    ...
    public void setUpdateStaticRecord(List<StaticRecord> ls){
        for (StaticRecord staticRecord : ls) {
            ...
            staticRecord.setUpdateTime(DateUtils.nowInMillis()/1000);
    ...
    public boolean isSEO(){
        ...
        List<SeoRecord> ls  =seoRecordOldMapper.findByPage(paged);
        if(ls.size()!=0){
...
//	        });
        ...
        if(!"fault".equals(message)){//http请求异常！
            ...
            if(!dataObj.has("result")){//没有错误
                for (int i = 0; i < urls.size(); i++) {
                    ...
                    seoRecordOldMapper.insertOrUpdate(record);
                }
            }
            else{
    ...
    public Boolean build360BaseSiteMap() throws ParseException, IOException {
        ...
        //记录site map生成开启
        configOldMapper.saveConfigType(configKey, GENERATE_START);
        ...
        //记录site map生成结束
        configOldMapper.saveConfigType(configKey, GENERATE_END);
        return true;
    }
    ...
    public Boolean build360ArticleSiteMap() throws IOException {
        ...
        List<Find> list = findOldMapper.findByPage(paged);
    ...
    public Boolean build360CompositionSiteMap() throws IOException {
        ...
        paged.setPageSize(MAX_SITE_MAP_CONTENT);
        List<Composition> list = compositionOldMapper.compositionByPage(paged);
    ...
    private BlockingQueue<Integer> getPageQueue() throws InterruptedException {
        int totalNum = goodsOldMapper.selectTotal();
        ...
        public void run(){
            try {
                while(!queue.isEmpty()){
                    ...
                    List<Goods> list = goodsOldMapper.findMidByPage(paged);
    ...
    private Map<String, Object> get360Config(String configKey){
        ...
        Config config = configOldMapper.selectByKey(configKey);
        if(config !=null){
