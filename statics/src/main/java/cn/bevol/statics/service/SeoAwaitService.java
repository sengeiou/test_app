...
import cn.bevol.statics.dao.mapper.SeoAwaitOldMapper;
...
public class SeoAwaitService {
	...
	@Resource
	SeoAwaitOldMapper seoAwaitOldMapper;
	@Resource
	...
	public int insertOrupdate(String mid, String url, String dataType, String dataSource, String operateType){
		...
		seoAwait.setOperateType(operateType);
		seoAwait.setDataType(dataType);
		return seoAwaitOldMapper.insertOrUpdate(seoAwait);
	}
	...
	public Paged findByPage(String dataType, String dataSource, String operateType, String state, String beginTime, String endTime, Integer page){
		SeoAwait seoAwait =new SeoAwait();
		 ...
		 if (!StringUtils.isEmpty(dataType)) {
		  ...
		  paged.setResult(seoAwaitOldMapper.findByPage(paged));
		 paged.setTotal(seoAwaitOldMapper.selectTotal(paged));
		return paged;
	}
	...
	public int selectTotalByWhere(String dataType, String dataSource, String operateType, int state){
		return seoAwaitOldMapper.selectTotalByWhere(dataType,dataSource,operateType);
	}
	
	public String addSeoBatchJob(){
		String result="";
		 if (conf.getResourceNum("seo_job_switch")!=0){
			 List<SeoAwait> ls = seoAwaitOldMapper.findAll();
			 return seoBatch(ls);
		 }
		 return result;
	}
	...
	public String addSeoBatchJob(Integer beginTime, Integer endTime){
		String result="";
		if (conf.getResourceNum("seo_job_switch")!=0) {
			List<SeoAwait> ls = seoAwaitOldMapper.findByDate(beginTime, endTime);
			return seoBatch(ls);
		}
		return result;
	}
			...
			if (remain > 0) {
				if (!dataObj.has("result")) {//没有错误
					for (int i = 0; i < urls.size(); i++) {
						SeoAwait seoAwait = new SeoAwait();
						seoAwait.setState(1);
						seoAwait.setUpdateTime(DateUtils.nowInMillis() / 1000);
						seoAwait.setUrl(urls.get(i));
						seoAwait.setDataType(dataType);
						seoAwait.setOperateType(operateType);
						seoAwaitOldMapper.update(seoAwait);
					}
				}
			}
		}
