...
public class CronService {
     ...
     * 产品计算执行任务数量
     */
    public static BlockingQueue<cn.bevol.model.GoodsTask> task=new ArrayBlockingQueue<cn.bevol.model.GoodsTask>(3);
    static{

        task.add(new cn.bevol.model.GoodsTask("task-1"));

        task.add(new cn.bevol.model.GoodsTask("task-2"));

        task.add(new cn.bevol.model.GoodsTask("task-3"));
    }

    /**
     * 增长处理的任务
     */
    public static Map<String, cn.bevol.model.GoodsTask> tasking=new HashMap<String, cn.bevol.model.GoodsTask>();
    ...
    public ReturnData batchUpdateByGoodsId(String sql,int sts,int rws,int pbath, Integer type) {
        cn.bevol.model.GoodsTask gt=task.poll();
        if(gt==null) {
